// index.js — Pokédex chatbot API (CommonJS, safe-port + static UI)
...
// Use 0 so CodeSandbox picks a free port and avoids conflicts
const PORT = process.env.PORT || 0;

/* ---------- tiny cache helper (be nice to PokéAPI) ---------- */
const cache = new Map();
...
async function getPokemon(nameOrId) {
  return cachedJSON(
    `${API}/pokemon/${encodeURIComponent(String(nameOrId).toLowerCase())}`
  );
}
async function getSpecies(nameOrId) {
  return cachedJSON(
    `${API}/pokemon-species/${encodeURIComponent(
...
async function getType(typeName) {
  return cachedJSON(
    `${API}/type/${encodeURIComponent(String(typeName).toLowerCase())}`
  );
}
...
// Build weaknesses/resistances from live type relations
async function weaknessesForPokemon(p) {
  ...
  for (const t of p.types.map((t) => t.type.name)) {
    const typeData = await getType(t);
...
// Evolution chain → readable paths
function flattenChain(chain) {
  ...
  function walk(node, acc) {
    ...
    if (!node.evolves_to || node.evolves_to.length === 0) {
      ...
      node.evolves_to.forEach((n) => walk(n, next));
    }
  }
  walk(chain.chain, []);
  return paths;
...
// Tiny regex “router” for natural-ish questions
function routeIntent(q) {
  ...
  const evo = text.match(
    /(evolutions?|evo(?: line)?|evolution of) ([a-z0-9\-\s\.]+)/
  );
  ...
  const typeQ = text.match(
    /(what (is|are) )?(the )?types? of ([a-z0-9\-\s\.]+)/
  );
  if (typeQ) return { intent: "types", name: typeQ[4].trim() };

  const move = text.match(
    /(what does|info on|details for) move ([a-z0-9\-\s\.]+)/
  );
...
/* ------------------- Routes ------------------- */
// Health/status (we moved the old "/" message here so "/" serves the UI)
app.get("/status", (_req, res) => {
  res.send("Pokédex Bot API is running. Try /ask?q=weaknesses%20of%20Garchomp");
});
...
// Chat-style answers (plain text)
app.get("/ask", async (req, res) => {
  const q = String(req.query.q || "");
  const intent = routeIntent(q);

  // tiny helpers for nice sentences
  const cap = (s) => (s ? s[0].toUpperCase() + s.slice(1) : s);
  const oxford = (arr, conj = "and") =>
    arr.length <= 1
      ? arr.join("")
      : arr.length === 2
      ? arr.join(` ${conj} `)
      : `${arr.slice(0, -1).join(", ")}, ${conj} ${arr[arr.length - 1]}`;
  const fmtMult = (m) => (Number.isInteger(m) ? `×${m}` : `×${m.toFixed(2)}`);

  try {
    if (intent.intent === "move") {
      const move = await cachedJSON(
        `${API}/move/${encodeURIComponent(intent.move)}`
      );
      const parts = [];
      ...
      const line2 = [
        move.power ? `Power ${move.power}` : null,
        move.accuracy ? `Accuracy ${move.accuracy}` : null,
    ...
    if (intent.intent === "stats") {
      ...
      const line =
        `${name}'s base stats — HP ${stats.hp}, Atk ${stats.attack}, Def ${stats.defense}, ` +
    ...
    if (intent.intent === "weaknesses") {
      ...
      parts.push(
        w.length
          ? `Weak to ${oxford(w)}.`
          : `${name} has no notable weaknesses.`
      );
    ...
    if (intent.intent === "evolution") {
      ...
      return res.send(
        unique.length
          ? `${name}'s evolution line: ${unique.join(" | ")}.`
          : `${name} does not evolve.`
      );
    }

    // Default dex-y summary
    const types = p.types.map((t) => cap(t.type.name));
    const abilities = p.abilities.map((a) => cap(a.ability.name));
    return res.send(
      `${name} (#${p.id}) is ${oxford(types, "and")} type. ` +
        `Abilities: ${oxford(abilities)}. Height ${p.height}, weight ${
          p.weight
        }.`
    );
  } catch (e) {
...
app.listen(PORT, () => {
  console.log(`Pokédex bot listening on port ${PORT}`);
});
